...
local function luau_deserialize(bytecode, luau_settings)
	...
	local function readFloat()
		local value = buffer_readf32(stream, cursor)
		cursor += 4
		return value
	end

	local function readDouble()
		local value = buffer_readf64(stream, cursor)
		cursor += 8
		return value
	end
	...
	local function readVarInt64()
		...
		for _ = 1, 10 do
			...
			result += bit32_band(value, 0x7F) * multiplier
	...
	if luauVersion < 3 or luauVersion > 14 then
	...
	local function readInstruction(codeList)
		...
		local opcode = bit32_band(
			luau_settings.decodeOp(value),
			0xFF
		)
		...
		if opinfo == nil then
			error(
				"unsupported opcode: " .. tostring(opcode),
				0
			)
		end
		...
		elseif opmode == 4 then -- AD
			...
			local temp = bit32_band(
				bit32_rshift(value, 16),
				0xFFFF
			)

			inst.D = if temp < 0x8000
				then temp
				else temp - 0x10000
	...
	local function checkkmode(inst, k)
		...
		if kmode == 1 then -- AUX
			...
			local index = inst.aux

			-- GETUDATAKS / SETUDATAKS / NAMECALLUDATA
			-- use only lower 16 bits for constant index.
			if inst.opcode == 83
				or inst.opcode == 84
				or inst.opcode == 85
			then
				index = bit32_band(index, 0xFFFF)
			end
		...
		elseif kmode == 4 then -- AUX import
			...
			local count = bit32_rshift(
				extend,
				30
			)

			local id0 = bit32_band(
				bit32_rshift(extend, 20),
				0x3FF
			)
			...
			if count == 2 then
				...
				local id1 = bit32_band(
					bit32_rshift(extend, 10),
					0x3FF
				)
		...
		elseif kmode == 5 then -- AUX boolean

			inst.K =
				bit32_extract(inst.aux, 0, 1) == 1

			inst.KN =
				bit32_extract(inst.aux, 31, 1) == 1

		elseif kmode == 6 then -- AUX number
		...
		elseif kmode == 8 then -- AUX low 16
	...
	local function readProto(bytecodeid)
		...
		-- v12+ has a serialized proto size
		if luauVersion >= 12 then
			protoSize = readVarInt()
			protoStart = cursor
			protoEnd = protoStart + protoSize
		end
		...
		for i = 1, sizek do
			...
			elseif kt == 8 then
				...
				local shape = {
					__tableWithConstants = true;
				}

				for j = 1, dataLength do
					local key = readVarInt()
					local constant = readInt32()
			...
			elseif kt == 10 then
				...
				local shape = {
					__classShape = true;
					className = className;
					properties = table_create(propertyCount);
					methods = table_create(methodCount);
				}

				for j = 1, propertyCount do
					shape.properties[j] = readVarInt()
				end

				for j = 1, methodCount do
					shape.methods[j] = readVarInt()
				end
		...
		if lineinfoenabled then
			...
			local lineinfo =
				table_create(sizecode)

			local abslineinfo =
				table_create(intervals)
		...
		-- v12+:
		--
		-- Do NOT try to guess which optional feature sections
		-- follow debug information.
		--
		-- protoSize gives us the exact end of this proto.
		-- This safely skips feedback/cost/class-related tail data.
		-- ========================================================

		if protoEnd ~= nil then

			if cursor > protoEnd then
				error(
					"proto data exceeds declared size",
					0
				)
			end
	...
	local mainProto =
		protoList[readVarInt() + 1]

	assert(
		cursor == buffer_len(stream),
		"deserializer cursor position mismatch"
	)
...
local function luau_load(module, env, luau_settings)
	...
	local alive = true 
		local closureProto = setmetatable({}, {
		__mode = "k"
	})
	...
	if template and template.__tableWithConstants then
		for _, item in template do
			if type(item) == "table" and item.key ~= nil then
				local key = constants[item.key + 1]

				if item.constant >= 0 then
					serialized[key] =
						constants[item.constant + 1]
				else
					serialized[key] = nil
				end
			end
    ...
    local loopInst = code[pc + inst.D]
    if generalized_iterators[loopInst] == nil then 
        ...
        generalized_iterators[loopInst] = coroutine_create(gen_iterator)
				...
				elseif op == 83 then --[[ GETUDATAKS ]]
					stack[inst.A] = stack[inst.B][inst.K]
					pc += 1 -- AUX

				elseif op == 84 then --[[ SETUDATAKS ]]
					stack[inst.B][inst.K] = stack[inst.A]
					pc += 1 -- AUX

				elseif op == 85 then --[[ NAMECALLUDATA ]]
					local A = inst.A
					local B = inst.B
					local kv = inst.K
				...
				elseif op == 86 then --[[ NEWCLASSMEMBER ]]
					error(
						"NEWCLASSMEMBER is not supported by this VM",
						0
					)

				elseif op == 87 then --[[ CALLFB ]]
					if interruptHook then
						interruptHook(
							stack,
							debugging,
					...
					pc += 1 -- AUX / feedback slot

				elseif op == 88 then --[[ CMPPROTO ]]
					local closure = stack[inst.A]

					local protoId = nil

					if closure ~= nil then
						protoId = closureProto[closure]
					end

					if protoId ~= inst.aux then
						pc += inst.D
					else
						pc += 1 -- AUX
					end
				...
				elseif op == 89 then --[[ FASTPCALL ]]
					-- FASTPCALL is an optimization hint.
					-- The following CALL still performs the real pcall/xpcall.
					-- Therefore this VM can safely skip the optimized path.
		...
		closureProto[result] = proto.bytecodeid

	return result
end
print("DEBUG luau_newsettings =", luau_newsettings)
return {
